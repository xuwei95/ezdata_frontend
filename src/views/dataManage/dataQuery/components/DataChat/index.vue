...
<script setup lang="ts">
  ...
  import { useScroll } from '@/components/jeecg/AiChat/hooks/useScroll';
  import { EventSourcePolyfill } from 'event-source-polyfill';
  import { ConfigEnum } from '/@/enums/httpEnum';
  import { getToken } from '/@/utils/auth';
  import { getAppEnvConfig } from '/@/utils/env';
  import { buildUUID } from '@/utils/uuid';
  import chatMessage from '@/components/jeecg/AiChat/components/chatMessage.vue';
    ...
    const initEventSource = () => {
      ...
      if (typeof EventSource !== 'undefined') {
        ...
        evtSource = new EventSourcePolyfill(
          `${VITE_GLOB_API_URL}/llm/data/chat?message=${message}${props.model_id ? '&model_id=' + props.model_id : ''}${
            options.parentMessageId ? '&topicId=' + options.parentMessageId : ''
          }`,
          {
